...
def build_order_checklist(state: StrategyState) -> str:
    ...
    lines = ["", "🔔 [주문 체크리스트]"]
    if mode == "IDLE":
        lines.append("□ 첫 주문은 시장가 매수로 입력")
        lines.append("□ 1회 매수 예산 확인")
...
def build_order_confirmation_table(state: StrategyState, rate: float) -> str:
    ...
    if mode == "IDLE":
        ...
        lines.append("\n🟢 [새 사이클 첫 매수]")
        lines.append("🛒 시장가 매수 | 주문가격: 시장가 | 수량: 증권앱에서 현재가 기준으로 정수주 입력")
        lines.append(f"💵 1회 매수 예산: {format_usd(first_attempt_amount)}")
        lines.append("📥 체결 후 봇 입력: '전량매수 체결' → 실제 체결가와 체결수량 입력")
        lines.append("⚠️ 첫 주문은 전일종가 +10% LOC가 아니라 시장가 매수 기준입니다.")
        return "\n".join(lines)
...
def build_plan_text(state: StrategyState, rate: float) -> str:
    ...
    if mode == "IDLE":
        ...
        buy_box = compact_lines(
            [
                ["🟢 첫매수", "시장가", "정수주", usd(first_attempt_amount)]
            ],
            [10, 8, 6, 8]
        )
        ...
        return (
            ...
            f"🛒 <b>첫 매수 미션</b>\n"
            f"<pre>{buy_box}</pre>\n"
            f"💡 첫 주문은 <b>전일종가+10% LOC가 아니라 시장가 매수</b>입니다.\n"
            f"📥 체결 후 <b>'전량매수 체결'</b>로 실제 체결가와 수량을 입력하세요.\n"
            f"기준 환율: ₩{rate:.2f}/$"
        )
